...
const criar_card = (card_jogadores) => {
    let jogador = card_jogadores.map(function (jog){
        return `
        <div class='card'>
            <a href="./outra.html">
                <button class="jogador" onclick="criar_cookie(${jog.ID})" >
                    <div class="foto">
                        <img src = "${jog.imagem}" class="imagem"></img>
                    </div>
                    <h2 class="nome">${jog.nome}</h2>
                    <h3 class="posicao">${jog.posicao}</h3>
                </button>
            </a>
        </div>
        `;
...
const criar_cookie = (index) => {
    document.cookie = "jogador="+index;
    location.reload();
}

const pesquisar = () => {
    const textpes = document.querySelector('#textpes').value;

    const jogadoresFiltrados = jogadores.filter((jog) =>
            jog.nome.toLowerCase().includes(textpes) ||
            jog.posicao.toLowerCase().includes(textpes)
    );
    criar_card(jogadoresFiltrados);

}

window.onload = () => {
    criar_card(jogadores)
};
